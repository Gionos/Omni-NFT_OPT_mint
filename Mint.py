...
if not os.path.isfile('config_user.json'):
    ...
    min_mint = input("Enter the minimum number of tokens to mint: ")
    max_mint = input("Enter the maximum number of tokens to mint: ")
...
def OmniNFT(private_key):
    ...
    w3 = Web3(HTTPProvider("https://mainnet.optimism.io"))
...
def remove_key_from_file(key):
    """Удаляет приватный ключ из файла после успешного выпуска токенов."""
    with open('private_keys.txt', 'r') as f:
        lines = f.readlines()
...
for key in private_keys:
    ...
    print(f"Wait {plaseholder} second before next operation...")
